...
Ts = 0.2;
sysD = zpk(1/0.45,[0.25,0.42],0.42,Ts);
...
main_folder_name = "1-5-4-indirect-MV-NMP";
...
deg_desA = len_desA - 1;
deg_desB = len_desB;
A_estimated = [1,-0.2,0.2]; % initial conditions
B_estimated = [0.5, -0.5];
...
R_real = [1.0000   43.2375];
S_real = [-100.3988   24.0208];
...
theta_hat_toPlot = zeros([num_samples, length(theta_real_toplot)]);
...
for i = skip_instances:num_samples
    ...
    A_dio = A_estimated;
    B_dio = B_estimated; 
    D_dio = conv([1 , zeros([1,d-1])], C_estimated); % RHS of the diophantin eqn.
