...
main_folder_name = "1-8-1-minVar-overparameter";
...
B_estimated = [0.01, 0.035, 0.1];
% RLS initial parameters (C)
theta_epsilon_zero = [0.1,0.1,0.2].';
...
R_calculated = [1,-0.1, 0.1]; 
S_calculated = [1,0.1, 0.1];
 

% R S which were calculated in the last part
R_real = [0.4200, -0.1890, 0];
S_real = [1.0700, 0.0950, 0];
...
theta_real_toplot = [A(2:end).'; 0; B.';0; noise_poly(2:end);0];
...
theta_hat_toPlot = zeros([num_samples, len_desA + len_desB + length(noise_poly(2:end))]);
